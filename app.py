...
def fetch_financials(ticker, current_bond_yield=4.4):
    try:
        ...
        return {
            ...
            "News": ""  # Will fill later via API if available
        }
    ...
    if not tickers:
        ...
        if results:
            ...
            for r in df_sorted.to_dict(orient="records"):
                try:
                    graham_val_num = float(r['Graham Value'].replace('$','').split()[0])
                except:
                    graham_val_num = None

                try:
                    graham_num_num = float(r['Graham Number'].replace('$','').split()[0])
                except:
                    graham_num_num = None

                valuation_note = ""
                if graham_val_num is not None:
                    diff_pct = ((r['Current Price Num'] - graham_val_num) / graham_val_num) * 100
                    if diff_pct > 0:
                        valuation_note = f"potentially overvalued by {diff_pct:.1f}% ❌"
                    else:
                        valuation_note = f"potentially undervalued by {abs(diff_pct):.1f}% ✅"

                st.markdown(f"**{r['Company Name']} ({r['Ticker']})**")
                st.markdown(f"**Industry Note:** Operates in the {r['Sector']} sector.")

                if graham_val_num is None or graham_num_num is None:
                    st.markdown(
                        f"**Valuation Insight:** {r['Company Name']} is trading at ${r['Current Price Num']:.2f}. Graham metrics not available."
                    )
                else:
                    st.markdown(
                        f"**Valuation Insight:** {r['Company Name']} is trading at ${r['Current Price Num']:.2f}, "
                        f"compared to Graham Number ${graham_num_num:.2f} and Graham Value ${graham_val_num:.2f}, "
                        f"indicating it is {valuation_note}."
                    )
                ...
                st.markdown(f"**Recent News:** {r['News'] if r['News'] else 'No recent news available.'}")
                st.markdown("---")
