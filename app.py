...
def fetch_news(ticker):
    try:
        ...
        news_data = resp.json()
        headlines = [d.get("headline") for d in news_data[:3] if d.get("headline")]
    ...
    if not tickers:
        ...
        if results:
            ...
            for _, r in df_sorted.iterrows():
                ...
                val_note = "potentially overvalued because price is above Graham Number and Graham Value" \
                    if gn != "N/A" and gv != "N/A" else "Graham metrics not available"

                st.markdown(f"""
**{r['Company Name']} ({r['Ticker']})**

Industry Note: Operates in the {info.get('industry','N/A')} sector.

Valuation Insight: {r['Company Name']} is trading at ${price}, {val_note}.
