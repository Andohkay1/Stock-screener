...
def fetch_financials(ticker, current_bond_yield=4.4):
    try:
        ...
        return {
            ...
            "Strength Note": f"Current Assets cover Total Liabilities, allowing acquisition without paying for fixed assets or property.",
...
if st.button("🚀 Run Screener"):
    if not tickers:
        ...
        if results:
            ...
            # Sort so Graham metrics are last
            col_order = [c for c in df.columns if c not in ["Graham Number", "Graham Value"]] + ["Graham Number", "Graham Value"]
            df_sorted = df[col_order].sort_values("Passed Count", ascending=False)
            ...
            for r in results:
                try:
                    ...
                    gn = parse_money(r["Graham Number"]) if r["Graham Number"] != "N/A" else None
                    gv = parse_money(r["Graham Value"]) if r["Graham Value"] != "N/A" else None

                    if gn and gv:
                        if price > gn and price > gv:
                            val_text = f"trading at ${price:.2f}, above its Graham Number (${gn:.2f}) and Graham Value (${gv:.2f}), potentially overvalued ❌."
                        elif price < gn and price < gv:
                            val_text = f"trading at ${price:.2f}, below its Graham Number (${gn:.2f}) and Graham Value (${gv:.2f}), potentially undervalued ✅."
                        else:
                            val_text = f"trading at ${price:.2f}. Graham metrics do not indicate clear over- or undervaluation."
                    else:
                        val_text = f"trading at ${price:.2f}. Graham metrics not available."
...
**Valuation Insight:** {r['Company Name']} is {val_text}
