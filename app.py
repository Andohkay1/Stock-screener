...
def fetch_news(symbol):
    ...
    try:
        ...
        headlines = [d.get("headline") for d in res[:3]] if isinstance(res, list) else []
...
def fetch_financials(ticker, current_bond_yield=4.4):
    try:
        ...
        eps_growth = ((eps_values[-1]-eps_values[0])/eps_values[0]) if len(eps_values)>=2 and eps_values[0]>0 else 0
...
if st.button("🚀 Run Screener"):
    if not tickers:
        ...
        if results:
            ...
            st.dataframe(df_sorted.drop(columns=["Industry", "Recent News"]))
            ...
            for r in results:
                try:
                    ...
                    insight = f"trading at ${price:.2f}"
                    metrics = []
                    if gn_val and price > gn_val:
                        metrics.append(f"above its Graham Number ({gn_val})")
                    if gv_val and price > gv_val:
                        metrics.append(f"and Graham Value (${gv_val:.2f})")
                    if metrics:
                        insight += ", " + " ".join(metrics) + ", indicating potential overvaluation ❌."
                    else:
                        insight += "."

                    st.markdown(f"### {r['Company Name']} ({r['Ticker']})")
                    st.markdown(f"**Industry Note:** Operates in the {r['Industry']} sector.")
                    st.markdown(f"**Valuation Insight:** {r['Company Name']} is {insight}")
