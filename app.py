...
def parse_money(value):
    try:
        return float("".join(c for c in str(value) if c in "0123456789."))
    except:
        return None
    ...
    if not tickers:
        ...
        if results:
            ...
            for _, r in df_sorted.iterrows():
                ...
                gn_text = f"above its Graham Number (${gn_val:.2f})" if gn_val and price > gn_val else ""
                gv_text = f"and Graham Value (${gv_val:.2f})" if gv_val and price > gv_val else ""
                over_val_text = f"potentially overvalued ❌" if (gn_val and gv_val and price > gn_val and price > gv_val) else ""
...
Valuation Insight: {r['Company Name']} is trading at ${price:.2f}. {gn_text} {gv_text}, {over_val_text}
