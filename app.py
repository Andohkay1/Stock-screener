...
# ====== FINNHUB API ======
FINNHUB_API_KEY = "d5gqckpr01qll3dk0t60d5gqckpr01qll3dk0t6g"  # replace with your key
...
def fetch_news(ticker, limit=3):
    ...
    try:
        ...
        if isinstance(data, list) and data:
            news_items = [f"{item.get('headline')} ({item.get('source')})" for item in data[:limit]]
            return "\n- " + "\n- ".join(news_items)
...
if st.button("🚀 Run Screener"):
    if not tickers:
        ...
        if results:
            ...
            # ===== TABLE =====
            st.dataframe(df_sorted.drop(columns=["Company Name","Sector","News"]))  # drop memo-only cols
            
            # ===== MEMOS =====
            st.markdown("### Investment Memos")
            for r in results:
                # Valuation logic
                gn = float(r['Graham Number'].split()[0]) if r['Graham Number'] != "N/A" else None
                gv = float(r['Graham Value'].split()[0]) if r['Graham Value'] != "N/A" else None
