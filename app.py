...
def fetch_news(ticker):
    try:
        ...
        if isinstance(resp, list) and resp:
            headlines = [d.get("headline", "") for d in resp[:5]]
...
if st.button("🚀 Run Screener"):
    if not tickers:
        ...
        if results:
            ...
            for r in results:
                try:
                    gn = parse_money(r["Graham Number"].split()[0]) if r["Graham Number"] != "N/A" else None
                    gv = parse_money(r["Graham Value"].split()[0]) if r["Graham Value"] != "N/A" else None
                    price = parse_money(r["Price"])

                    # Valuation Insight text
                    if gn and gv and price:
                        val_insight = f"{r['Ticker']} is potentially overvalued ❌ because the price is above its Graham Number and Graham Value."
                    else:
                        val_insight = f"{r['Ticker']} – Graham metrics not available."

                    industry_summary = r['Business Summary'].split('.')[0]  # first sentence
                    industry_note = f"Operates in the {r['Industry']} sector, {industry_summary}."

                    strength_note = "Current Assets pay all Total Liabilities."

                    st.markdown(f"""
**{r.get('longName', r['Ticker'])} ({r['Ticker']})**
